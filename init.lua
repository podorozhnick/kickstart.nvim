...
do
  ...
  -- Incremental selection using Neovim 0.12's built-in treesitter selection.
  -- (`main`-branch nvim-treesitter no longer ships its own; core provides
  --  visual `an`/`in` to grow to parent / shrink to child -- see
  --  `:help treesitter-incremental-selection`.) These keep the old muscle
  --  memory: <C-space> starts/grows the selection, <BS> shrinks it back.
  --  NOTE: remap = true is required so the `an`/`in` in the RHS trigger the
  --  built-in treesitter selection mappings (which are themselves keymaps);
  --  without it they're treated as literal motions and won't grow/shrink.
  vim.keymap.set('n', '<C-space>', 'van', { remap = true, desc = 'Init/expand selection (treesitter)' })
...
do
  ...
  -- tokyonight is installed/configured but not activated; catppuccin-latte is
  -- the active colorscheme (with per-highlight tweaks applied by recolor.nvim,
  -- see lua/custom/plugins/recolor.lua + recolor.json).
  -- vim.cmd.colorscheme 'tokyonight-day'

  vim.pack.add { gh 'catppuccin/nvim' }
  vim.cmd.colorscheme 'catppuccin-latte'

  -- oxocarbon is available as an alternative light theme; installing it also
  -- keeps the background set to light.
...
do
  ...
  require('conform').setup {
    ...
    format_on_save = function(bufnr)
      -- Format on save for everything except languages without a well
      -- standardized style (or where the LSP formatter is undesirable).
...
-- ============================================================
-- SECTION 9b: GIT UI (Neogit)
-- ============================================================
do
  -- Magit-like git interface, with diffview integration.
  vim.pack.add {
    gh 'NeogitOrg/neogit',
    gh 'nvim-lua/plenary.nvim', -- required (already installed for telescope)
...
-- Extra local options (cursor shapes, etc.)
